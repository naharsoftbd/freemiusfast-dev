...
class FreemiusLicenseService
{
    ...
    public function getLicenseByUser($userId)
    {
        ...
        $fsUserId = UserFsEntitlement::where('email', $user->email)->first()->fs_user_id;

        return FreemiusLicense::where('fs_user_id', $fsUserId)->where('product_id', $this->productId)->get();
    }
